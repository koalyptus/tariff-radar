...
import { projectDataDir, projectRoot } from "@tariff-radar/shared";
...
describe("projectDataDir", () => {
  it("resolves the data directory under the workspace root", () => {
    const root = mkdtempSync(join(tmpdir(), "shared-test-"));
    try {
      const nested = join(root, "packages", "some-pkg", "src");
      mkdirSync(nested, { recursive: true });
      const entry = join(nested, "index.ts");
      writeFileSync(entry, "");
      expect(projectDataDir(pathToFileURL(entry).href)).toBe(join(root, "data"));
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
